...
public class MainActivity extends AppCompatActivity implements View.OnClickListener {
    ...
    private void postFix(String ip) {
        ...
        for (int i = 0; i < sep.length; i++) {
            if (sep[i].equals("x") || sep[i].equals("(") || sep[i].equals("+") || sep[i].equals("-") || sep[i].equals("/") || sep[i].equals("%")) input.push(sep[i]);
            else if (sep[i].equals(")")) {
                while (!input.peek().equals("(")) post_input.add(input.pop());
                input.pop();
            } else post_input.add(sep[i]);
        }
        while (!input.isEmpty()) post_input.add(input.pop());
    }
    private void calc() {
        makeEmpty();
    ...
    private void trig_calc(String ip) {
        ...
        if (ip.startsWith("sin")) et_input.setText(Double.toString(Math.sin(Math.toRadians(val))));
        else if (ip.startsWith("cos")) et_input.setText(Double.toString(Math.cos(Math.toRadians(val))));
        else if (ip.startsWith("log")) et_input.setText(Double.toString(Math.log(val)));
    }
